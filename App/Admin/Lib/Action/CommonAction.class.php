...
class CommonAction extends Action
{
	...
	protected function uploadImages($water = 0, $dir = 'images',$is_thumb = false,$whs = array(),$is_swf = false)
	{
		...
		if ($is_thumb)
			$save_rec_Path = "./Www/Uploads/" . $dir . "/" . toDate(gmtTime(), 'Ym/d') . "/origin/"; //上传至服务器的相对路径  
		else
			$save_rec_Path = "./Www/Uploads/" . $dir . "/" . toDate(gmtTime(), 'Ym/d') . "/"; //上传至服务器的相对路径  
		
		$save_path = ROOT_PATH . '/' . $save_rec_Path; //绝对路径
