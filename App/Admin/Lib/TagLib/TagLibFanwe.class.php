...
class TagLibFanwe extends TagLib
{
    ...
    public function _editor($attr)
    {
        ...
        switch(strtoupper($type)) {
            case 'CKEDITOR':
                $parseStr   =	'<script type="text/javascript" src="__STATIC__/Admin/Ckeditor/ckeditor.js"></script><script type="text/javascript" src="__STATIC__/Admin/Ckfinder/ckfinder.js"></script><textarea id="'.$id.'" name="'.$name.'">'.$content.'</textarea><script type="text/javascript">var '.$id.' =CKEDITOR.replace("'.$id.'",{"width":"'.$width.'","height":"'.$height.'","toolbar":"'.$toolbar.'"}) ;CKFinder.setupCKEditor('.$id.',"__STATIC__/Admin/Ckfinder") ;</script>';
                break;
            default :
    ...
    public function _list($attr)
    {
        ...
        foreach($fields as $field) {//显示指定的字段
			...
			if(in_array($property[0],$nosort))
            	...
            	$parseStr .= '<a href="javascript:sortBy(\''.$property[0].'\',\'{$sort}\',\''.ACTION_NAME.'\')" title="'.L('ACCORDING').$showname[2].'{$sortType} ">'.$showname[0].'<eq name="order" value="'.$property[0].'" ><img src="__STATIC__/Admin/Images/{$sortImg}.gif" align="absmiddle"></eq></a></th>';
			}
        }
